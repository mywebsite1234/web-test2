...
// like games stuff
/**
 * Load likedGames from localStorage (migrating old object format),
 * always returning an array (newest-first).
 */
...
/**
 * Save an array of game-names (newest-first) back to storage.
 * @param {string[]} games
 */
function saveLikedGames(games) {
  localStorage.setItem('likedGames', JSON.stringify(games));
}

/**
 * Check if a game is currently liked.
 * @param {string} name
 * @returns {boolean}
 */
function isLiked(name) {
  const games = loadLikedGames();
  return games.includes(name);
}

/**
 * Like a game: move it to front, dedupe, cap length.
 * @param {string} name
 * @param {number} [maxLikes=40]
 * @returns {string[]} the updated queue
 */
function likeGame(name, maxLikes = 40) {
  let games = loadLikedGames().filter(n => n !== name);
  games.unshift(name);
  if (games.length > maxLikes) {
    games = games.slice(0, maxLikes);
  }
  saveLikedGames(games);
  return games;
}

/**
 * Unlike a game: remove it from wherever it is.
 * @param {string} name
 * @returns {string[]} the updated queue
 */
function unlikeGame(name) {
  const games = loadLikedGames().filter(n => n !== name);
  saveLikedGames(games);
  return games;
}

/**
 * Toggle like/unlike, update the icon, and return the new queue.
 * @param {string} name
 * @param {HTMLImageElement} imgEl – the <img> element
 * @param {{likedSrc: string, unlikedSrc: string}} icons
 * @param {number} [maxLikes=40]
 * @returns {string[]} the updated queue
 */
function toggleLike(name, imgEl, icons, maxLikes = 40) {
  if (isLiked(name)) {
    const updated = unlikeGame(name);
    imgEl.src = icons.unlikedSrc;
    return updated;
  } else {
    const updated = likeGame(name, maxLikes);
    imgEl.src = icons.likedSrc;
    return updated;
  }
}
//
...
function setupActionButtons() {
    ...
    likeBtn.addEventListener('click', async (e) => {
        ...
        toggleLike(gameName, e.target.firstChild, {
            likedSrc: likedIcon,
            unlikedSrc: notLikedIcon
        });
...
function getRandomImageUrl(urls) {
    // select ad image
  let randomIndex = 0; //Math.floor(Math.random() * urls.length);
...
window.addEventListener('load', async () => {
    ...
    // dont skip for crawler
    //if (retrievedGames.length > 0 && retrievedGames[0].name == "TemplateGameName")
    //{
        //retrievedGames = retrievedGames.slice(1); // skip first element because it's template
    //}
    ...
    // update metadata
    let metaDesc = gameData.metadescription; //gameData.description.length > 155 ? gameData.description.substr(0, 156) : gameData.description;
    let gameDesc2 = gameData.description;

    if (gameData.description.endsWith(".html"))
    {
        const htmlgameRes = await fetch("/assets/gamesdesc/" + gameData.description);
        gameDesc2 = await htmlgameRes.text();
        //metaDesc = gameDesc2.length > 155 ? gameDesc2.substr(0, 156) : gameDesc2;
    }

    let gameControls = gameData.controls;

    if (gameData.controls.endsWith(".html"))
    {
        const htmlControlsRes = await fetch("/assets/gamesdesc/" + gameData.controls);
        gameControls = await htmlControlsRes.text();
    }
    ...
    document.getElementById('description').innerText = gameDesc2;
    const controlsEl = document.getElementById('controls');
    if (controlsEl) controlsEl.innerHTML = gameControls;

    const developerEl = document.getElementById('developer');
    if (developerEl) developerEl.innerText = `${gameName} was created by ${gameData.developer}.`;

    if (gameData.article != null) {
        let artRes = await fetch("/assets/gamesdesc/" + gameData.article);
...
if (localRun == false)
{
    document.addEventListener('DOMContentLoaded', async () => {
        //let retrievedGamesRes = await fetch('../../assets/ts_games.json');
        let retrievedGamesRes = await fetch('../../assets/ts_games_small.json');
        retrievedGames = await retrievedGamesRes.json();
        let gameData = retrievedGames.find(game => game.name === gameName);

        if (gameData.length > 0 && gameData[0].name == "TemplateGameName")
        {
            gameData = gameData.slice(1);
        }
...
async function suggestGames(gameData) {
    ...
    for (let gameD of suggestedGames) {
        ...
        gamesDiv.innerHTML += `
            <li name="${game}" id="gameDiv" onclick="handleGameClick('${game}')" class="${classlist}">
                <div class="imageCon">
                    <img src="${data.image}" alt="FreezeNova.Cloud ${game}" title="FreezeNova.Cloud ${game}"/>
                </div>
...
function addArrowListeners() {
    ...
    arrowLeft.addEventListener('click', () => {
        ...
        toggleArrowVisibility(); // Check visibility after scroll
    });
    ...
    // Add scroll event listener to toggle visibility during scroll
    gamesCon.addEventListener('scroll', toggleArrowVisibility);
